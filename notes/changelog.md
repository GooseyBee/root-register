...
| 날짜 | 작업 |
|---|---|
| 2026-09-26 | [Claude 정밀 검토 · 맞춤 문장 · 주간 리포트](#2026-09-26--claude-정밀-검토--맞춤-문장--주간-리포트) |
| 2026-09-26 | [알림 테스트는 누른 기기로만](#2026-09-26--알림-테스트는-누른-기기로만) |
| 2026-09-26 | [앱 이름을 누르면 펼친 것 모두 접기](#2026-09-26--앱-이름을-누르면-펼친-것-모두-접기) |
...
## 2026-09-26 · Claude 정밀 검토 · 맞춤 문장 · 주간 리포트

**요청**: AI 피드백을 Gemini 실시간 확인과 Claude 확인(버튼 → 희주 폰 알림 → Claude에 "피드백 확인해줘") 둘로 나누고, 더 좋은 방법이 있으면 제안해 달라.

**한 일**
- [x] 번역 카드에 버튼 두 개: **⚡ AI에게 바로 묻기**(Gemini, 몇 초) / **🔍 Claude 정밀 검토**
- [x] 정밀 검토 요청 → 희주에게 휴대폰 알림. 처리는 **Claude 루틴이 하루 3번(8:5x·12:5x·20:5x)** 자동으로 해요. 급하면 "피드백 확인해줘"
- [x] Claude 답에 **Gemini 검수 결과**: "Gemini 설명 검수 ✓" 또는 "Gemini 설명을 바로잡았어요"
- [x] 답이 달리면 DB 트리거(`notify_claude_answer`)가 요청한 사람에게 알림, 🔔에 "Claude 정밀 검토 도착"
- [x] 주간 루틴이 오답 노트를 읽고 사람마다 **나를 위한 문장** 2~4개(`drills.for_author`, `focus`)를 섞어 넣어요. 카드에 칩으로 보이고 새 묶음 안에서 맨 위
- [x] **📋 이번 주 리포트**(`weekly_reports`): 자주 틀린 것 3가지, 좋아진 점, 맞춤 문장 안내. 넣으면 알림, 번역 연습 탭 위에 카드
- [x] 지시서: [`notes/routines/claude-review.md`](routines/claude-review.md) 새로 씀, [`weekly-content.md`](routines/weekly-content.md)에 2-1 맞춤 문장·2-2 주간 리포트 추가
- [x] 마이그레이션 `20260926060000_claude_review_personal.sql`, Edge Function `assist` v8

**왜**
- 원래 아이디어대로면 희주가 매번 알림을 받고 직접 중계해야 해요. 알림은 그대로 두고 처리를 루틴에 맡겨서 사람 손을 뺐어요.
- 무료 모델의 설명이 가끔 틀리는데, 그게 오답 노트에 그대로 쌓이면 잘못 배워요. Claude가 검수하면 두 모델의 장점(속도·깊이)을 같이 가져가요.
- 오답 노트는 모으기만 해선 쓸모가 없어요. 틀린 패턴을 다시 연습하게 해야 고쳐져요(간격 반복). 그래서 맞춤 문장과 리포트로 피드백 → 연습 루프를 닫았어요.
- 비용은 그대로 0원이에요. Gemini는 무료 티어, Claude는 구독에 포함된 루틴이에요.

## 2026-09-26 · 알림 테스트는 누른 기기로만
- 질문(희주): 데스크톱 크롬에서 알림 테스트를 누르면 왜 폰으로 알림이 오나?
- 원인: 알림은 **사람 단위**로 보내요. 희주 님 이름으로 기기 두 대(데스크톱 크롬 `fcm.googleapis.com`, 아이폰 `web.push.apple.com`)가 등록돼 있어서, 테스트도 두 기기 모두로 갔어요. 데스크톱에 안 보인 건 컴퓨터의 알림 설정(macOS 시스템 설정 → 알림 → Google Chrome, 또는 집중 모드)이 막고 있을 가능성이 커요.
- 고친 것: 테스트 버튼은 **누른 기기로만** 보내요. 앱이 이 기기의 구독 주소(endpoint)를 함께 보내고, 함수 `pushTo()`가 그 주소로만 걸러요. 실제 알림(피드백, 답장, 건의 완료, 새 콘텐츠)은 지금처럼 그 사람의 모든 기기로 가요.
- 테스트 후 안내: "이 기기로 보냈어요. 안 보이면 컴퓨터·폰의 알림 설정을 확인해 주세요." 이 기기가 등록돼 있지 않으면 끄기 → 켜기를 안내해요.

## 2026-09-26 · 앱 이름을 누르면 펼친 것 모두 접기
- 요청(희주): 코멘트, 지난 문장처럼 열렸다 접히는 것들이 앱 이름을 눌러 첫 화면으로 올 때는 기본으로 접혀 있게.
- 전에는 제목을 눌러도 펼침 상태가 그대로 남아서 첫 화면이 길게 열려 있었어요.
- `collapseAll()`: 코멘트, 답장 입력칸, 지난 문장 목록과 각 문장, 오늘의 문장 뜻, 추천 표현, 안내 상자, 용어 뜻 보기, 알림 창을 모두 접고, 지난 문장 "더 보기"도 처음 20개로 돌려요.
- 그대로 두는 것: 쓰던 입력칸 내용, 수정 중인 칸. 실수로 제목을 눌러도 쓰던 글이 날아가지 않게요.
- 탭을 옮길 때는 지금처럼 펼침 상태가 유지돼요. 초기화는 제목(앱 이름)을 누를 때만이에요.
- 확인: 가짜 데이터에서 뜻·코멘트·지난 문장·지난 문장 하나를 펼친 뒤 다른 탭 → 제목 클릭 → 모두 접힘, 주소 `/`, 오류 0건.
