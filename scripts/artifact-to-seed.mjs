...
for (const t of load("terms"))
  out.push(`insert into public.terms (id, word, pair, meaning, examples, register, domain, lang, added_by, created_at) values (${qs(t.id)}, ${qs(t.word)}, ${qs(t.pair)}, ${qs(t.meaning)}, ${j(exs(t))}, ${qs(t.register || "문어")}, ${qs(t.domain || "일반")}, ${qs(t.lang)}, null, ${ts(t.createdAt)}) on conflict (id) do nothing;`);
