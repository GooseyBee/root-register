...
- **번역 연습**: 매일 하는 짧은 문장 연습이에요(하루 1~2문장 권장). **매주 월요일 오전 새 문장이 방향마다 12개씩** 올라와요(Claude 루틴, [지시서](notes/routines/weekly-content.md)). 방향은 한→영, 영→한 두 가지이고 주제는 마케팅·광고, 비즈니스·이메일, 일상 회화, 여행 표현이에요(방향마다 24문장). 처음 열면 희주는 한→영, 혜림은 영→한이 보여요(`members.drill_dir`). 내가 제출해야 상대 번역이 보이고(블라인드), 내가 제출하면 추천 표현과 포인트가 열려요. 번역 연습 코멘트는 그 문장을 제출한 사람에게만 보여요(스포 방지). 제출한 번역에 **⚡ AI에게 바로 묻기**를 누르면 Gemini 무료 API가 몇 초 안에 설명을 달고, **🔍 Claude 정밀 검토**를 누르면 Claude 루틴이 하루 3번 더 깊게 봐요. 매주 월요일에는 오답 노트로 **나를 위한 문장**과 **이번 주 리포트**가 와요. 자주 틀리는 표현은 **내 오답 노트**에 모여요(아래 [AI 피드백과 알림](#ai-피드백과-알림)).
- **교차번역** (예전 이름: 세션 기록 → 함께 번역): 다음 회차 원문 6개와 추천 표현은 같은 루틴이 약 2주마다(마지막 회차 뒤 10일 이상 지나면) 올려요. 2주에 한 번 모임용 원문을 각자 블라인드로 번역하고, 모임에서 비교해 합의안과 메모를 적어요. 원문마다 난이도(쉬움·보통·실전)가 있어요. 내 번역을 저장하면 추천 표현이 열리고, 번역 연습처럼 Claude 피드백을 요청할 수 있어요.
...
- **즐겨찾기(★)와 코멘트**: 용어, 교차번역, 카피, 오늘의 문장, 번역 연습, 건의마다 달 수 있어요. 코멘트에는 쓴 날짜와 시간이 보이고, 코멘트마다 **답장**(한 단계)과 **좋아요(♡)**를 달 수 있어요. 답장이 달리면 휴대폰 알림이 가요. 즐겨찾기는 사람마다 따로 저장되고, 목록마다 "즐겨찾기만 보기"가 있어요.
- **건의함**: 앱을 어떻게 바꾸고 싶은지 올리는 게시판이에요. Claude Code 세션에서 "건의함 확인해줘"라고 하면 Claude가 새 건의를 읽고 요약해 줘요. 희주 님이 승인하면 반영하고, 상태와 처리 메모를 달아요. 건의가 **완료**되면 올린 사람에게 휴대폰 알림과 🔔 알림이 가요(DB 트리거 → `assist`). 상태와 메모는 앱에서 바꿀 수 없고 Claude만 SQL로 갱신해요.
...
notes/
  ...
  style/korean-voice.md  한글 표현 스타일 가이드 (분야별 톤, 피할 상투어, 번역투)
  routines/weekly-content.md  주간 콘텐츠 루틴 지시서
```
...
- 답장(`comments.parent_id`)은 트리거가 대상을 부모에서 복사하고 한 단계로 모아요. 그래서 번역 연습 코멘트의 공개 규칙이 답장에도 적용돼요.
- 건의 완료 알림: `requests` 상태가 done이 되면 트리거가 pg_net으로 `assist`를 불러요(`hook_secret`으로 인증). 올린 사람은 `done_seen_at`만 고칠 수 있어요.
...
번역 피드백은 **세 단계**예요. 빠른 답은 Gemini가, 깊은 검토는 Claude가, 한 주의 패턴 정리는 주간 루틴이 맡아요.

| 단계 | 버튼 / 시점 | 누가 | 걸리는 시간 | 비용 | 무엇을 |
|---|---|---|---|---|---|
| ⚡ 바로 | **AI에게 바로 묻기** | Gemini 무료 API | 몇 초 | 없음 | 틀린 곳과 이유를 짧게 |
| 🔍 정밀 | **Claude 정밀 검토** | Claude 루틴 (하루 3번) | 최대 반나절 | 구독에 포함 | 격·뉘앙스까지 깊게 + Gemini 설명이 맞는지 검수(✓ / 바로잡음) |
| 📋 주간 | 매주 월요일 | 주간 콘텐츠 루틴 | 한 주 | 구독에 포함 | 오답 노트로 **나를 위한 문장** 2~4개 + **이번 주 리포트** |

```mermaid
flowchart LR
  U["제출한 번역"] -->|⚡ 바로 묻기| R[("feedback_requests")]
  U -->|🔍 정밀 검토| CR["claude_requested_at"]
  R --> F["Edge Function assist"] --> G["Gemini 무료 API"]
  G --> A["answer (by=gemini)"] --> P["🔔 + 휴대폰 알림"]
  G --> M[("mistake_notes")]
  CR --> P2["희주에게 알림"]
  CR --> RT["Claude 루틴 8:5x·12:5x·20:5x<br/>(급하면 '피드백 확인해줘')"]
  RT --> CA["claude_answer + verdict"] -->|DB 트리거| P
  RT --> M
  M --> W["월요일 루틴"] --> D["drills.for_author·focus<br/>나를 위한 문장"]
  W --> WR[("weekly_reports")] -->|DB 트리거| P
```

**왜 이렇게 나눴나**
- 처음 생각은 "버튼 → 희주 폰에 알림 → 희주가 Claude에 와서 요청"이었어요. 그러면 희주가 매번 사람 손으로 중계해야 해서, 같은 알림은 보내되 **처리는 루틴이 하루 3번 자동으로** 하게 했어요. 급할 때만 희주가 "피드백 확인해줘"라고 하면 돼요.
- Claude가 Gemini 답을 **검수**하면 무료 모델의 틀린 설명이 오답 노트에 쌓이는 걸 막을 수 있어요(`claude_verdict`: `ok` / `fixed`).
- 오답 노트를 모으기만 하면 쓸모가 없어요. 그래서 주간 루틴이 그 사람의 실수 패턴을 겨냥한 문장을 섞어 넣고, 리포트로 정리해 줘요. 피드백 → 연습 → 다시 피드백이 한 바퀴 돌아요.

| 경우 | 알림 |
|---|---|
| Gemini 답 도착 | 요청한 사람 "피드백이 도착했어요", 희주 "OO 님이 피드백을 요청했어요" |
| Gemini 실패 (키 없음, 한도 초과) | 희주 "자동 피드백 실패". 정밀 검토 루틴이 대신 답해요 |
| 정밀 검토 요청 | 희주 "OO 님이 Claude 정밀 검토를 요청했어요" |
| 정밀 검토 도착 | 요청한 사람 "Claude 정밀 검토가 도착했어요" (Gemini 검수 결과 포함) |
| 주간 리포트 | 그 사람 "이번 주 리포트가 도착했어요" |
| 새 건의 | 희주 "새 건의" |

- 모델은 무료 티어 Flash 계열을 차례로 시도해요(`GEMINI_MODEL` 시크릿 → gemini-3-flash → gemini-3-flash-preview → gemini-2.5-flash). 404면 다음 후보로 넘어가요.
...
- 루틴 지시서: [정밀 검토](notes/routines/claude-review.md) · [주간 콘텐츠·맞춤 문장·리포트](notes/routines/weekly-content.md). 희주가 "피드백 확인해줘"라고 할 때도 정밀 검토 지시서를 그대로 따라요.
...
| 날짜 | 작업 |
|---|---|
| 2026-09-26 | [Claude 정밀 검토 · 맞춤 문장 · 주간 리포트](notes/changelog.md#2026-09-26--claude-정밀-검토--맞춤-문장--주간-리포트) |
| 2026-09-26 | [알림 테스트는 누른 기기로만](notes/changelog.md#2026-09-26--알림-테스트는-누른-기기로만) |
| 2026-09-26 | [앱 이름을 누르면 펼친 것 모두 접기](notes/changelog.md#2026-09-26--앱-이름을-누르면-펼친-것-모두-접기) |
| 2026-09-26 | [한글 표현 개편 (스타일 가이드, 마케팅 카피 톤)](notes/changelog.md#2026-09-26--한글-표현-개편-스타일-가이드-마케팅-카피-톤) |
| 2026-09-26 | [코멘트 좋아요](notes/changelog.md#2026-09-26--코멘트-좋아요) |
